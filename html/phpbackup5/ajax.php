...
/*
		...
		if (responsetextarray[responsetextarray.length-1].indexOf("100% of ")!=-1 && responsetextarray[1].indexOf(".mp4")!=-1) {
			endinit2("mp4");
		}

		if (responsetextarray[responsetextarray.length-1].indexOf("100% of ")!=-1 && responsetextarray[1].indexOf(".webm")!=-1) {
			endinit2("webm");
		}

*/
	
	document.getElementById("debuging").innerHTML+="<br>"+isdonetext+isdone;
	
	if (isdone && isdonetext.indexOf(".m4a")!=-1) {
		endinit("m4a");
	}
	if (isdone && isdonetext.indexOf(".ogg")!=-1) {
		endinit("ogg");
	}
}
...
xmlhttp.open("GET","ytdl.php?url="+document.getElementById("sendurl").value+"&audioonly="+audioonly,true);
xmlhttp.send();
}
...
xmlhttp.onreadystatechange=function()
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
	...
	encodeurl=encodeurl.replace("\'","'");
    document.getElementById("downloadid").innerHTML+='<p><a href="youtube/'+encodeurl+'">Download</a></p><p><a href="ajax.php?audioonly='+audioonly+'">Download Another</a></p>';
	window.location.assign("#bottom");    
	}
  }
xmlhttp.open("GET","movem4a.php?bdir="+basedirectory+"&oggorm4a="+oggorm4a+"&audioonly="+audioonly,true);
xmlhttp.send();
}
...
if (window.XMLHttpRequest)
  {// code for IE7+, Firefox, Chrome, Opera, Safari
  xmlhttpisdone=new XMLHttpRequest();
  }
else
  {// code for IE6, IE5
  xmlhttpisdone=new ActiveXObject("Microsoft.XMLHTTP");
  }
xmlhttpisdone.onreadystatechange=function()
  {
  if (xmlhttpisdone.readyState==4 && xmlhttp.status==200)
    {
		
		if (xmlhttpisdone.responseText.length>0) {
			isdone=true;
			isdonetext=xmlhttpisdone.responseText;
...
xmlhttpisdone.open("GET","checkdone.php?bdir="+bdir,true);
xmlhttpisdone.send();
}
...
<p>Audio Only <input type="checkbox" value="yes" id="audioonly" <?php if ($_GET['audioonly']=="true" || $_GET['audioonly']==null) { echo "checked"; } ?>></p>
